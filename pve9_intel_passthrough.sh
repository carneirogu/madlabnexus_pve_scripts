...
run_menu() {
  ...
  if [[ "$ENABLE_SRIOV" == "1" ]]; then
    menu_str SRIOV_IFACE "NIC interface for SR-IOV" "${SRIOV_IFACE}"
    menu_str SRIOV_VFS "Number of VFs to create" "${SRIOV_VFS}"
  fi
...
[Service]
Type=oneshot
ExecStart=/bin/sh -c 'echo ${SRIOV_VFS:-8} > /sys/class/net/%i/device/sriov_numvfs'
...
EOF
  ...
  if (( ENABLE_SRIOV )); then
    if [[ -e "/sys/class/net/${SRIOV_IFACE}" ]]; then
      systemctl enable --now "sriov@${SRIOV_IFACE}"
    else
      echo "NOTE: SR-IOV iface ${SRIOV_IFACE} not found; unit created but not enabled."
    fi
  fi
}
